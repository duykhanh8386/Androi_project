...
public interface ApiService {
    ...
    // ===== ⭐️ MỚI: TEACHER GRADE MANAGEMENT (Endpoint giả định) =====

    /**
     * (GIẢ ĐỊNH) Lấy danh sách điểm của cả lớp, đã được nhóm theo học sinh.
     * Trả về List<StudentGrades> để khớp với item_class_grade_row.xml
     */
    @GET("api/teacher/classes/{classId}/student-grades")
    Call<List<Grade>> getTeacherClassGrades(
            @Path("classId") int classId
    );
    /**
     * (GIẢ ĐỊNH) Cập nhật điểm cho một học sinh.
     * Gửi đi GradeUpdateRequest để khớp với dialog_add_edit_grade.xml
     */
    @POST("api/teacher/classes/students/{studentId}/grades")
    Call<MessageResponse> updateStudentGrades(
            @Path("studentId") Long studentId,
            @Body List<Grade> gradeRequest
    );
